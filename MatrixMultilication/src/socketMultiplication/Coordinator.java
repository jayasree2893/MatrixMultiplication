...
public class Coordinator implements Runnable{ 
	...
	/*public Coordinator(int n, int numNodes,int port) { 
		this(MatrixMultiple.createRandomMatrix(n),MatrixMultiple.createRandomMatrix(n),numNodes,port);
	}*/
	void configurate()
	{
 	...
 	void configurate(int portNum) { 
		try { 
			...
			for (int i=0; i<numNodes; i++ ) { 
				...
				int nodeXNum = dis.readInt(); 			//get worker ID
			...
			System.out.printf("Sent neighbour information to all workers\n");
	...
	public static void main(String[] args) { 
		...
		int [][] matA = MatrixMultiple.createRandomMatrix(matrixdim);
		int [][] matB = MatrixMultiple.createRandomMatrix(matrixdim);
